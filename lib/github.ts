...
function getAppConfig() {
  ...
  if (!key.includes("-----BEGIN")) {
    try {
      key = normalizePem(Buffer.from(privateKey, "base64").toString("utf8"));
    } catch {}
  }
...
function asn1IntZero(): Buffer {
  return Buffer.from([0x02, 0x01, 0x00]);
}
function asn1Null(): Buffer {
  return Buffer.from([0x05, 0x00]);
}
function asn1OidRsaEncryption(): Buffer {
  return Buffer.from([0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x01]);
}
function asn1OctetString(data: Buffer): Buffer {
